...
\begin{lstlisting}[language=JavaScript, caption={Standard API Response Format}]
// Success response
{
  success: true,
  data: { ... },
...
{
  success: false,
  error: { message: "Human-readable error", code: "ERROR_CODE" }
}
\end{lstlisting}
...
\textbf{Authentication Middleware Implementation:}

\begin{lstlisting}[language=JavaScript, caption={JWT Authentication Middleware (auth.middleware.js)}]
...
Figure~\ref{fig:auth_flow} shows the complete JWT authentication sequence including token refresh.
...
\begin{lstlisting}[language=JavaScript, caption={Payroll Computation Core (payroll.service.js)}]
async function computeEmployeePayslip(employeeId, month, year) {
  // 1. Fetch base salary structure
  const { rows: [salary] } = await db.query(
    'SELECT * FROM salary_structures WHERE employee_id = $1',
    [employeeId]
  );

  // 2. Count working days and absent days in month
  const workingDays = getWorkingDaysInMonth(month, year);
  const { rows: attendanceRows } = await db.query(
    `SELECT COUNT(*) as present FROM attendance
     WHERE employee_id = $1 AND EXTRACT(MONTH FROM date) = $2
     AND EXTRACT(YEAR FROM date) = $3 AND status = 'present'`,
    [employeeId, month, year]
  );
  const absentDays = workingDays - parseInt(attendanceRows[0].present);

  // 3. Compute daily rate deduction for absences
  const dailyRate = salary.basic_salary / workingDays;
  const absenceDeduction = dailyRate * absentDays;

  // 4. Apply FBR income tax slab calculation
  const annualSalary = salary.basic_salary * 12;
  const monthlyTax = calculateFBRTax(annualSalary, salary.is_filer) / 12;

  // 5. Apply EOBI (0.75%) and SESSI (0.75% up to PKR 25,000)
  const eobi = salary.basic_salary * 0.0075;
  const sessi = Math.min(salary.basic_salary, 25000) * 0.0075;

  // 6. Compute net salary
  const gross = salary.basic_salary + salary.allowances;
  const totalDeductions = monthlyTax + eobi + sessi + absenceDeduction;
  const netSalary = gross - totalDeductions;

  return { gross, monthlyTax, eobi, sessi,
           absenceDeduction, netSalary, absentDays };
}
\end{lstlisting}

Figure~\ref{fig:payroll_flow} illustrates the complete payroll computation flow for a single payroll run.
...
The FBR tax calculator in \texttt{taxCalculator.js} implements the 2024--25 tax slabs for both filers and non-filers:

\begin{lstlisting}[language=JavaScript, caption={FBR Tax Calculator (taxCalculator.js)}]
function calculateFBRTax(annualSalary, isFiler = true) {
  // FBR Tax Year 2024-25 slabs (filer rates)
  const slabs = [
    { max: 600000,    rate: 0,     base: 0       },
    { max: 1200000,   rate: 0.025, base: 0       },
    { max: 2200000,   rate: 0.125, base: 15000   },
    { max: 3200000,   rate: 0.225, base: 140000  },
    { max: 4100000,   rate: 0.275, base: 365000  },
    { max: Infinity,  rate: 0.35,  base: 612500  }
  ];

  const prevMax = [0, 600000, 1200000, 2200000, 3200000, 4100000];
  let tax = 0;

  for (let i = 0; i < slabs.length; i++) {
    if (annualSalary <= slabs[i].max) {
      tax = slabs[i].base +
            (annualSalary - prevMax[i]) * slabs[i].rate;
      break;
    }
  }

  // Non-filer surcharge: approximately 10% higher effective rate
  return isFiler ? tax : tax * 1.1;
}
\end{lstlisting}
...
\begin{lstlisting}[language=JavaScript, caption={Chatbot Intent Classification (chatbot.service.js)}]
function classifyIntent(message) {
  const msg = message.toLowerCase();
  if (/salary|pay|payslip|earn/.test(msg))
    return 'SALARY_QUERY';
  if (/leave balance|days left|remaining leave/.test(msg))
    return 'LEAVE_BALANCE';
  if (/attendance|present|absent|check.?in/.test(msg))
    return 'ATTENDANCE_STATUS';
  if (/policy|holiday|working hours/.test(msg))
    return 'HR_POLICY';
  if (/tax|fbr|eobi|deduction/.test(msg))
    return 'TAX_INFO';
  return 'GENERAL';
}

async function processMessage(employeeId, message) {
  const intent = classifyIntent(message);
  switch (intent) {
    case 'SALARY_QUERY':
      return await getSalaryResponse(employeeId);
    case 'LEAVE_BALANCE':
      return await getLeaveBalanceResponse(employeeId);
    case 'ATTENDANCE_STATUS':
      return await getAttendanceResponse(employeeId);
    default:
      return getGeneralResponse(intent);
  }
}
\end{lstlisting}
